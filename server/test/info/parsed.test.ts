...
describe('info/parsed.ts', () => {
	...
	describe('CompletionInfo.getKindMetadata', () => {
		...
		it('returns correct metadata for "channel_class_def"', () => {
			const entry = assertNonNull(completionInfo.getKindMetadata('channel_class_def'));
		...
		it('returns null for an unknown kind', () => {
			assert.strictEqual(completionInfo.getKindMetadata('nonexistent_kind'), null);
		});
		...
		it('knownKinds contains all expected kind strings', () => {
			const expected = [
				'reg_def', 'channel_class_def', 'type_def', 'func_def',
				'proc_def', 'macro_def', 'endpoint_def', 'message_def',
			];
			for (const k of expected) {
				assert.ok(completionInfo.knownKinds.includes(k), `"${k}" should be in knownKinds`);
			}
		});
	});
	...
	describe('CompletionInfo.getKeywordMetadata – single-variant', () => {
		...
		it('"import" has one variant whose description is back-filled from astNodeInfo', () => {
			...
			assert.ok(entry.variants[0].description !== null && entry.variants[0].description.includes('import'));
		});
		...
		it('returns null for an unknown keyword', () => {
			assert.strictEqual(completionInfo.getKeywordMetadata('unknown_kw_xyz'), null);
		});
	});
	...
	describe('CompletionInfo.getKeywordMetadata – multi-variant expansion', () => {
		it('"type" expands into 2 variants (all fields are arrays of length 2)', () => {
			...
			assert.ok(v1.description !== null && v1.description.includes('type parameter'));
	...
	describe('CompletionInfo – timing completions', () => {
		...
		it('getLifetimeTimingEntry("eternal") returns correct entry', () => {
			const entry = assertNonNull(completionInfo.getLifetimeTimingEntry('eternal'));
			...
			assert.ok(entry.description !== null && entry.description.includes('Eternal'));
		});
		...
		it('getLifetimeTimingEntry returns null for unknown key', () => {
			assert.strictEqual(completionInfo.getLifetimeTimingEntry('not_a_key'), null);
		});
	...
	describe('AstNodeInfo.getFrom – kind + type', () => {
		...
		it('"proc_def_body", "extern" → external process body', () => {
			const entry = assertNonNull(astNodeInfo.getFrom('proc_def_body', 'extern'));
	...
	describe('AstNodeEntry – internal flag', () => {
		it('is false for user-facing nodes', () => {
			assert.strictEqual(assertNonNull(astNodeInfo.getFrom('proc_def')).internal, false);
			assert.strictEqual(assertNonNull(astNodeInfo.getFrom('expr', 'binop')).internal, false);
		});

		it('is true for internal-only nodes', () => {
			assert.strictEqual(assertNonNull(astNodeInfo.getFrom('singleton_or_list')).internal, true);
			assert.strictEqual(assertNonNull(astNodeInfo.getFrom('singleton_or_list', 'single')).internal, true);
			assert.strictEqual(assertNonNull(astNodeInfo.getFrom('cycle_proc')).internal, true);
		});
	});
	...
	describe('API demonstration – sample lookups', () => {
		...
		it('can enumerate all control-flow keywords', () => {
			const controlKeywords = completionInfo.knownKeywords.filter(kw =>
				completionInfo.getKeywordMetadata(kw)?.variants.some(v => v.category === 'control')
			);
			for (const expected of ['loop', 'recv', 'send', 'if']) {
				assert.ok(controlKeywords.includes(expected), `"${expected}" should be a control keyword`);
			}
		});

		it('"generate_seq" description is accessible', () => {
			const entry = assertNonNull(completionInfo.getKeywordMetadata('generate_seq'));
	...
	describe('CompletionInfo – astNodeInfo description fallback', () => {
		it('keyword with astKind but no explicit description gets description from astNodeInfo', () => {
			...
			assert.strictEqual(v.description, astNodeInfo.getFrom('proc_def')?.description);
		});
		...
		it('keyword with astKind containing a "/" is resolved via kind+type lookup', () => {
			...
			assert.ok(v.description !== null, 'description should be back-filled');
			assert.strictEqual(v.description, astNodeInfo.getFrom('data_type', 'record')?.description);
		});
		...
		it('explicit description takes precedence over astNodeInfo fallback', () => {
			...
			assert.notStrictEqual(v.description, astNodeInfo.getFrom('thread')?.description);
		});
		...
		it('timing entry with astKind but no explicit description gets description from astNodeInfo', () => {
			...
			const raw = {
				...
				timingCompletions: {
					lifetime: {
						testlt: { hint: 'test', insertText: 'test', lspKind: 'TypeParameter', astKind: 'proc_def' },
					},
					sync: {
						testsc: { hint: 'test', insertText: 'test', lspKind: 'TypeParameter', astKind: 'reg_def' },
					},
				},
			};
			...
			assert.strictEqual(lt.description, astNodeInfo.getFrom('proc_def')?.description ?? null);
			const sc = assertNonNull(withNodeInfo.getSyncTimingEntry('testsc'));
			assert.strictEqual(sc.description, astNodeInfo.getFrom('reg_def')?.description ?? null);
		});
		...
		it('CompletionInfo constructed without nodeInfo leaves descriptions as-is', () => {
			...
			const raw = {
				kind: {},
				builtInKeywordCompletions: {
					testkw: { category: 'test', hint: 'test', lspKind: 'Keyword', astKind: 'proc_def' },
				},
			};
	...
	describe('AstNodeEntry – examples field', () => {
		...
		it('"compilation_unit" has a null examples field', () => {
			const entry = assertNonNull(astNodeInfo.getFrom('compilation_unit'));
			assert.strictEqual(entry.examples, null, 'compilation_unit should have no examples');
		});

		it('"sig_def" has a null examples field', () => {
			const entry = assertNonNull(astNodeInfo.getFrom('sig_def'));
			assert.strictEqual(entry.examples, null, 'sig_def should have no examples');
		});

		it('"proc_def" description does not contain "**Example:**"', () => {
			const entry = assertNonNull(astNodeInfo.getFrom('proc_def'));
			assert.ok(!entry.description.includes('**Example:**'), 'description should be prose-only');
		});

		it('"proc_def" examples field contains a fenced code block when non-null', () => {
			const entry = assertNonNull(astNodeInfo.getFrom('proc_def'));
			assert.ok(entry.examples !== null);
			assert.ok(entry.examples.includes('```'), 'examples should contain a fenced code block');
		});
	});
