...
const documentSettings = LazyMap.onCacheMissAsync<
  string,
  Thenable<AnvilServerSettings>
>(async (resource) => {
  if (!hasConfigurationCapability) {
    return Promise.resolve(globalSettings).then(
      (s) => s || DEFAULT_ANVIL_SERVER_SETTINGS,
    );
  }
  const result =
...
const documentAnvilManagers = LazyMap.onCacheMiss<
  string,
  AnvilDocument | undefined
>((resource) => {
...
documentSubscribers.onDidChangeTextDocument = (_e) =>
  connection.onDidChangeTextDocument((e) => {
    ...
    serverLogger.info(
      `Document ${e.textDocument.uri} changed with ${e.contentChanges.length} content changes.`,
    );
...
connection.onDidChangeConfiguration((change) => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = change.settings['anvil'] || globalSettings;
  }
  connection.languages.diagnostics.refresh();
...
connection.languages.diagnostics.on(async (params) => {
  ...
  const diagnostics = hasDiagnosticRelatedInformationCapability
    ? await AnvilDescriptionGenerator.describeDiagnostics(
        anvilDocument,
  ...
  serverLogger.info(
    `Diagnostics generated. ${diagnostics.length} problems. AST ${anvilDocument.anvilAst ? `available (${anvilDocument.isResultsUpToDate ? 'up-to-date' : 'outdated'}).` : 'not available.'}`,
  );
...
connection.onHover(async (params) => {
  serverLogger.info(
    'Hover event received at position',
    params.position,
    'in document',
    ...
    return !D
      ...
      : {
          contents: {
            kind: 'markdown',
            value:
              '**Error:** No information available (AnvilDocument not available)',
          },
        };
    ...
    return !D
      ...
      : {
          contents: {
            kind: 'markdown',
            value:
              '**Error:** No information available (AST information unavailable)',
          },
        };
  }
    ...
    return !D
      ...
      : {
          contents: {
            kind: 'markdown',
            value:
              '**Error:** No information available (cannot find AST node at position)',
          },
        };
  }
  ...
  return {
    contents: {
      kind: 'markdown',
      value: AnvilDescriptionGenerator.describeNode(
        node,
        anvilDocument,
...
connection.onDefinition(async (params) => {
  ...
  const node = anvilDocument.getClosestAnvilNodeToLspPosition(position);
  const identifierUnderCursor =
    anvilDocument.getClosestIdentifierToLspPosition(position);

  if (!node) {
    return null;
  }

  const allDefs = node.definitions;
  if (!allDefs || allDefs.length === 0) {
    serverLogger.info(`No definitions found for node at position`);
    return null;
  }

  const updatedDefs = allDefs.filter(
    (def) => !!ast.node(def)?.names.includes(identifierUnderCursor ?? ''),
  );
  ...
  return defs.map((def) => {
    return {
      ...
      range:
        documentAnvilManagers
          .get('file://' + def.fullpath)
          ?.getLspRangeOfAnvilSpan(def.span) ??
...
connection.onTypeDefinition(async (params) => {
  ...
  const updatedDefs = typeDefs.filter(
    (def) => !!ast.node(def)?.names.includes(identifierUnderCursor ?? ''),
  );
  ...
  serverLogger.info(
    `Found ${defs.length} type definition(s) for node at position`,
  );
...
connection.onReferences(async (params) => {
  ...
  return refs.map((ref) => {
    return {
      ...
      range:
        documentAnvilManagers
          .get('file://' + ref.fullpath)
          ?.getLspRangeOfAnvilSpan(ref.span) ??
...
connection.onCompletion(async (params) => {
  ...
  if (!anvilDocument) {
    serverLogger.info(
      `Completion request received for document ${params.textDocument.uri}, but AnvilDocument is not available.`,
    );
    return [];
  }

  const completionDetails = AnvilCompletionGenerator.getCompletions(
    params.position,
    anvilDocument,
  );
  ...
  serverLogger.info(
    `Found ${completionDetails.length} completion(s) for the current context.`,
  );

  return completionDetails.map((c) =>
    c.lspCompletionItem({ allowOOOSnippet: settings.snippets?.fancy }),
  );
});
...
connection.onCompletionResolve(async (item: CompletionItem) => {
  ...
  if (filepath && typeof filepath === 'string' && Array.isArray(nodepath)) {
    ...
    if (doc) {
      const node = doc.anvilAst
        ?.root(filepath)
        ?.unsafeTraverse(...nodepathResolved);
      if (node) {
        const defDesc =
...
connection.onSignatureHelp(async (params) => {
  ...
  if (!anvilDocument) {
    serverLogger.info(
      `Signature help request received for document ${params.textDocument.uri}, but AnvilDocument is not available.`,
    );
    return null;
  }
  ...
  if (!anvilDocument.anvilAst) {
    serverLogger.info(
      `AST not yet available for signature help in document ${params.textDocument.uri}`,
    );
    return null;
  }

  serverLogger.info(
    `Signature help requested at position ${params.position.line}:${params.position.character}`,
  );
  ...
  serverLogger.info(
    `Signature help result: ${result ? `${result.signatures.length} signature(s)` : 'null'}`,
  );
...
connection.languages.inlayHint.on(async (params) => {
  ...
  const inlayHints = AnvilInlayHintGenerator.generateInlayHints(
    anvilDocument,
    settings,
  );

  serverLogger.info(
    `Found ${inlayHints.length} inlay hint(s) for document ${uri}`,
  );
