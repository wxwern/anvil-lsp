...
export class AnvilCompletionDetail {
  ...
  public static create(
    ...
    hint: string,
    documentation?: { node?: AnvilAstNode; desc?: string },
    source: 'builtinKeyword' | 'astNode' = 'astNode',
  ) {
  ...
  public static snippetFromNode(
    label: string,
    insertText: string,
    node: AnvilAstNode,
  ): AnvilCompletionDetail {
    const typedef = node.resolveAs(AnvilTypeSchema);
    const useTypedefData =
      typedef?.type === 'record' || typedef?.type === 'variant';
    const entry =
      (useTypedefData
        ? completionInfo.getKindMetadata(
            typedef.data_type.kind,
  ...
  public lspCompletionItem(options?: {
    allowOOOSnippet?: boolean;
  }): CompletionItem {
    ...
    return {
      ...
      insertTextFormat: this.isSnippet
...
export class AnvilCompletionGenerator {
  ...
  public static readonly TYPEDEF_REGEX_GROUP =
    '([a-zA-Z_][a-zA-Z0-9_<>\\(\\)\\[\\]]*)';
  public static readonly LIFETIME_IDENTIFIER_REGEX_GROUP =
    '(#[~0-9]+|[#a-zA-Z_][a-zA-Z0-9_]*[+0-9]*)';
  ...
  private static getPrefixAtPosition(
    ...
    const prefix = document.textDocument.getText({
      start: {
        line: (options?.fast ?? true) ? position.line : 0,
        character: 0,
      },
      end: position,
    });
    ...
    completionLogger.info(
      `Prefix at position ${position.line}:${position.character} is: "${logSample.length < lastNChars ? logSample : '...' + logSample}"`,
    );
  ...
  public static getCompletions(
    ...
    const readRegCompletions = this.checkReadRegisterHeuristics(
      position,
      document,
    );
    ...
    const writeRegCompletions = this.checkWriteRegisterHeuristics(
      position,
      document,
    );
    ...
    const sendRecvCompletions = this.checkSendRecvHeuristics(
      position,
      document,
    );
    ...
    const timingAnnotCompletions = this.checkTimingAnnotHeuristics(
      position,
      document,
    );
    ...
    const constructSyntaxCompletions = this.checkConstructSyntaxHeuristics(
      position,
      document,
    );
    ...
    const typedefParamCompletions = this.checkTypedefParamHeuristics(
      position,
      document,
    );
    ...
    completionItems.push(
      ...this.getAllEntries(this.getAllNodes(position, document)),
    );
  ...
  private static checkReadRegisterHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    ...
    completionLogger.info(
      'Checking register completion heuristic with regex:',
      regex,
    );
    ...
    const matchingRegs = regs.filter((r) =>
      r.name?.startsWith(regPartialNamePrefix),
    );

    return matchingRegs.map((r) =>
      AnvilCompletionDetail.basicFromNode(r.name!, r),
    );
  }
  ...
  private static checkWriteRegisterHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    const regex = new RegExp(
      `${this.SPACER_REGEX_GROUP}set\\s+${this.IDENTIFIER_REGEX_GROUP}?$`,
      'g',
    );
    completionLogger.info(
      'Checking write register completion heuristic with regex:',
      regex,
    );
    const match = regex.exec(prefix);
    ...
    if (!match) {
      completionLogger.info(
        'Write register completion heuristic did not match.',
      );
      return null;
    }
    ...
    return matchingRegs.map((r) =>
      AnvilCompletionDetail.snippetFromNode(r.name!, r.name! + '$1 := $0', r),
    );
  }
  ...
  private static checkSendRecvHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    const regex = new RegExp(
      `${this.SPACER_REGEX_GROUP}(send|recv)\\s+(${this.IDENTIFIER_REGEX_GROUP}(\\.(${this.IDENTIFIER_REGEX_GROUP})?)?)?$`,
      'g',
    );
    completionLogger.info(
      'Checking send/recv completion heuristic with regex:',
      regex,
    );
    const match = regex.exec(prefix);
    if (!match) {
    ...
    completionLogger.info(
      `Parsed < | isSend: ${isSend} | "${endpointPartialNamePrefix}" | dot: ${hasDot} | "${messagePartialNamePrefix}" >`,
    );
    ...
    const matchingEndpoints =
      endpoints?.filter((e) => e.name?.startsWith(endpointPartialNamePrefix)) ||
      [];
    ...
    const matchingChannels =
      channels?.filter((c) =>
        c.names.find((n) => n.startsWith(endpointPartialNamePrefix)),
      ) || [];

    const endpointCandidates = [...matchingEndpoints, ...matchingChannels];

    completionLogger.info(
      `Found ${endpointCandidates.length} endpoint candidates for prefix "${endpointPartialNamePrefix}"`,
    );

    if (!messagePartialNamePrefix && !hasDot) {
      // If we haven't completed the endpoint, return endpoints.
      return endpointCandidates.flatMap((e) =>
        e.names.map((n) => AnvilCompletionDetail.basicFromNode(n!, e)),
      );
    }
    ...
    const messageCompletions: AnvilAstNode[] = [];
    for (const endpointCandidate of endpointCandidates) {
      const channelClassDef = endpointCandidate.definitions
      ...
      if (
        endpointCandidate.kind === 'channel_def' &&
        endpointCandidate.down('endpoint_right')?.resolve() ===
    ...
    completionLogger.info(
      `Found ${messageCompletions.length} message candidates for prefix "${messagePartialNamePrefix}"`,
    );

    return messageCompletions.map((m) =>
      AnvilCompletionDetail.snippetFromNode(
        m.name!,
        m.name! + (isSend ? '($1)$0' : ''),
        m,
      ),
  ...
  private static checkTimingAnnotHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    const match = regex.exec(prefix);
    completionLogger.info(
      'Checking timing annotation completion heuristic with regex:',
      regex,
    );
    if (!match) {
      completionLogger.info(
        'Timing annotation completion heuristic did not match.',
      );
      return null;
    }
    ...
    if (!hasFirstAnnot) {
      completionLogger.info(
        'Timing annotation completion heuristic did not match (first annotation not yet detected).',
      );
      return null;
    }

    completionLogger.info(
      `Parsed < | hasFirst: ${hasFirstAnnot} | "${firstAnnotPrefix}" | hasRangeStart: ${hasRangeStartAnnot} | "${rangeStartAnnotPrefix}" | hasRangeEnd: ${hasRangeEndAnnot} | "${rangeEndAnnotPrefix}" >`,
    );
    ...
    const messageDefs =
      ast
        ...
        ?.down('messages')
        .children.filter((n) => n.kind === 'message_def')
    ...
    if (!hasRangeStartAnnot) {
      ...
      for (const key of completionInfo.knownLifetimeTimingKeys) {
        ...
        if (key.includes('%s')) {
          ...
          for (const msgDef of messageDefs) {
            ...
            const description =
              entry.description?.replace(/%s/g, `\`${msgName}\``) ?? null;
            completionItems.push(
              AnvilCompletionDetail.create(
                hashReformat(label),
      ...
      const hashPresentInPrefix =
        (rangeStartAnnotPrefix.startsWith('#') && !hasRangeEndAnnot) ||
        rangeEndAnnotPrefix.startsWith('#');
      const H = !hashPresentInPrefix ? '#' : '';
      const hashReformat = (s: string) => {
        if (s.startsWith('#')) {
          return H + s.slice(1);
  ...
  private static checkTypedefHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    const regex = new RegExp(
      `^\\s*(reg|left|right|let)\\s+${this.IDENTIFIER_REGEX_GROUP}\\s*(:)?\\s*(\\()?${this.TYPEDEF_REGEX_GROUP}?$`,
      'g',
    );
    completionLogger.info(
      'Checking typedef completion heuristic with regex:',
      regex,
    );
    const match = regex.exec(prefix);
    ...
    if (!hasColon) {
      completionLogger.info(
        'Intentionally returning nothing (: not yet entered).',
      );
      return [];
    }

    const allDefs = this.getAllNodes(position, document, {
      filter: (n) =>
        !!(
          n.kind === 'type_def' &&
          n.names.find((n) => n.startsWith(typedefPartialNamePrefix))
        ),
    ...
    completionLogger.info(
      `Found ${allResults.length} typedef candidates for prefix "${typedefPartialNamePrefix}"`,
    );
    return allResults;
  }
  ...
  private static checkTypedefParamHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    completionLogger.info(
      'Checking typedef parameter completion heuristic with regex:',
      regex,
    );
    const match = regex.exec(prefix);
    ...
    if (!match) {
      completionLogger.info(
        'Typedef parameter completion heuristic did not match.',
      );
      return null;
    }
    ...
    const typeDefs = this.getAllNodes(position, document, {
      filter: (n) => !!(n.kind === 'type_def' && n.name === typeName),
    }).filter((n) => n.kind === 'type_def');

    if (typeDefs.length === 0) {
      completionLogger.info(
        `No type definitions found for type name "${typeName}"`,
      );
      return [];
    }
    ...
    completionLogger.info(
      `Found ${matchingParams.length} typedef parameter candidates for prefix "${paramPartialPrefix}"`,
    );
    ...
    const snippet =
      '<' + matchingParams.map((p, i) => `\${${i + 1}:${p}}`).join(', ') + '>';
    ...
    return [
      AnvilCompletionDetail.create(
        ...
        snippet.slice(
          1,
        ) /* remove the leading < since it's already in the regex matching rule */,
        CompletionItemKind.TypeParameter,
        `type parameters`,
        { node: typeDef },
      ),
  ...
  private static checkSpawnHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    completionLogger.info(
      'Checking spawn completion heuristic with regex:',
      regex,
    );
    ...
    const procs = this.getAllNodes(position, document, {
      filter: (n) =>
        !!(n.kind === 'proc_def' && n.name?.startsWith(procPartialNamePrefix)),
    });
    ...
    completionLogger.info(
      `Found ${results.length} proc candidates for prefix "${procPartialNamePrefix}"`,
    );
    return results;
  }
  ...
  private static checkCallHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    completionLogger.info(
      'Checking call completion heuristic with regex:',
      regex,
    );
    ...
    const funcs = this.getAllNodes(position, document, {
      filter: (n) =>
        !!(n.kind === 'func_def' && n.name?.startsWith(funcPartialNamePrefix)),
    });
    ...
    const results = funcs.map((f) => {
      ...
      return AnvilCompletionDetail.snippetFromNode(name, name + argFormat, f);
    });

    completionLogger.info(
      `Found ${results.length} func candidates for prefix "${funcPartialNamePrefix}"`,
    );
    return results;
  }
  ...
  private static checkConstructSyntaxHeuristics(
    position: Position,
    document: AnvilDocument,
  ): AnvilCompletionDetail[] | null {
    const prefix = this.getPrefixAtPosition(position, document);
    ...
    const regex = new RegExp(
      `${this.SPACER_REGEX_GROUP}${this.TYPEDEF_REGEX_GROUP}(:(:({|${this.IDENTIFIER_REGEX_GROUP})?)?)?$`,
      'g',
    );
    completionLogger.info(
      'Checking construct syntax completion heuristic with regex:',
      regex,
    );
    const match = regex.exec(prefix);
    if (!match) {
      completionLogger.info(
        'Construct syntax completion heuristic did not match.',
      );
      return null;
    }
    ...
    for (const typeDef of typeDefs) {
      switch (typeDef.down('data_type').down('type').resolve()) {
        case 'record': {
          ...
          let fieldTemplate =
            '::{' +
            fieldNames.map((n, i) => `${n} = \${${i + 1}}`).join('; ') +
            '}';

          completionLogger.info(
            `Found ${fieldNames.length} field candidates for prefix "${memberPartialPrefix}"`,
          );
        ...
        case 'variant': {
          ...
          completionLogger.info(
            `Found ${matchingVariants.length} variant candidates for prefix "${memberPartialPrefix}"`,
          );
        ...
        default: {
          completionLogger.info(
            `Type definition "${typeName}" is not a record or variant, no construct syntax completions available.`,
          );
          break;
        }
      }
  ...
  private static getAllNodes<T = any, S = any>(
    ...
    let nodeList: AnvilAstNode<unknown>[] = relnodepath
      ? ((scoped
          ? Array.of(
              ...((
                ast.closestNode(
          ...
          : ast
              .getAllRoots()
              .flatMap(
                (n) => n.unsafeTraverse(...relnodepath).children,
  ...
  private static getAnvilBuiltinCompletions(
    category?: string | null,
    filter?: string[],
  ): AnvilCompletionDetail[] {
    const completionItems: AnvilCompletionDetail[] = [];
    ...
    for (const keyword of completionInfo.knownKeywords) {
      ...
      completionItems.push(
        ...AnvilCompletionDetail.fromKeyword(keyword, category),
      );
    }
