...
export class AnvilDocument {
  ...
  public async compile(settings: AnvilServerSettings): Promise<boolean> {
    ...
    this._compileLock = (async () => {
      ...
      const compiler = new AnvilCompiler(
  ...
  public async scheduleCompileDebounced(
  ...
  public getClosestIdentifierToLspPosition(position: Position): string | null {
    ...
    while (
      c > 0 &&
      (characters.length === 0 ||
        characters[characters.length - 1]?.match(/[a-zA-Z0-9_]/))
    ) {
      c--;
      characters.push(
        document.getText({
    ...
    return characters
      .filter((x) => x)
      .filter((x) => x?.match(/[a-zA-Z0-9_]/))
      .join('');
  }

  public getClosestAnvilNodeToLspPosition(
    position: Position,
  ): AnvilAstNode | null {
    if (!this._anvilAst) return null;

    const res = this.applyTrackedEditsOnPositionInstance(position, {
      reverse: true,
    });
    if (!res.valid) return null;

    position = res.position;
    const anvilLoc = AnvilLspUtils.lspPosToAnvilPos(position);

    return this._anvilAst.closestNode(
      this.filepath,
      anvilLoc.line,
      anvilLoc.col,
    );
  ...
  public getLspRangeOfAnvilSpan(span: AnvilSpan): Range | null {
    ...
    const rs = this.applyTrackedEditsOnPositionInstance(range.start, {
      reverse: false,
    });
    const re = this.applyTrackedEditsOnPositionInstance(range.end, {
      reverse: false,
    });
  ...
  private applyTrackedEditsOnPositionInstance(
    position: Position,
    options: { reverse: boolean },
  ): { position: Position; valid: boolean } {
    if (!this.EXPERIMENTAL_TRACK_POST_AST_TEXT_EDITS)
      return { position: position, valid: true };
